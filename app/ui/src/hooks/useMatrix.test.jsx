...
describe('useMatrix', () => {
  ...
  it('POSTs the filter to /api/matrix/data and exposes returned rows + counts', async () => {
    const authFetch = makeAuthFetch({
      '/api/matrix/data': {
        ...
        resourceContexts: [{ resourceId: 'r1', contexts: [{ id: 'c1', displayName: 'Finance', contextType: 'Tag', variant: 'manual' }] }],
        rowType: 'principal',
        subjectCount: 2,
        subjectTotal: 10,
        resourceCount: 3,
    ...
    expect(result.current.resourceContexts).toEqual([
      { resourceId: 'r1', contexts: [{ id: 'c1', displayName: 'Finance', contextType: 'Tag', variant: 'manual' }] },
    ]);
  ...
  it('populates rollup state when the response is a roll-up payload', async () => {
    ...
    expect(result.current.data).toEqual([]);
    // Roll-up payloads carry no per-resource sidecar — it must reset.
    expect(result.current.resourceContexts).toEqual([]);
