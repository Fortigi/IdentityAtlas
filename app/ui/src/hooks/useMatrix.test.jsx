...
describe('useMatrix', () => {
  ...
  it('POSTs the filter to /api/matrix/data and exposes returned rows + counts', async () => {
    const authFetch = makeAuthFetch({
      '/api/matrix/data': {
        ...
        resourceContexts: [{ resourceId: 'r1', contexts: [{ id: 'c1', displayName: 'Finance' }] }],
        rowType: 'principal',
        subjectCount: 2,
        subjectTotal: 10,
        resourceCount: 3,
    ...
    // Contexts sidecar for the grid's Contexts column.
    expect(result.current.resourceContexts).toEqual([
      { resourceId: 'r1', contexts: [{ id: 'c1', displayName: 'Finance' }] },
    ]);
  ...
  it('populates rollup state when the response is a roll-up payload', async () => {
    ...
    expect(result.current.data).toEqual([]);
    // Roll-up views have no per-resource rows, so the contexts sidecar clears.
    expect(result.current.resourceContexts).toEqual([]);
