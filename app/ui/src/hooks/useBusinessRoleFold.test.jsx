...
describe('analyseRoleRows', () => {
  ...
  // A matrix that does not show business roles as rows — and a grid still
  // loading — hands this an empty or absent row list. It has to answer "nothing
  // is foldable" rather than invent a row: every fold affordance in the grid is
  // keyed off these three, so a phantom entry is a chevron on a row that isn't
  // there.
  it('finds nothing to fold in a grid with no rows', () => {
    for (const rows of [null, undefined, []]) {
      const { foldableRoles, rolesByChild, childCounts, roleNames } = analyseRoleRows(rows, childrenByRole);
      expect(foldableRoles.size).toBe(0);
      expect(rolesByChild.size).toBe(0);
      expect(childCounts.size).toBe(0);
      expect(roleNames.size).toBe(0);
    }
  });
...
describe('buildRoleLayout', () => {
  ...
  // The layout runs on every render, including the ones before any row has
  // arrived. An empty grid must stay empty — laying out a phantom row here puts
  // it on screen, since this output IS what the grid renders.
  it('lays out an empty grid as an empty grid', () => {
    for (const rows of [null, undefined, []]) {
      const out = buildRoleLayout(rows, analysis, new Set());
      expect(out.visibleRows).toEqual([]);
      expect(out.roleFoldInfo.size).toBe(0);
      expect(out.foldedChildRows.size).toBe(0);
    }
  });

  // A role whose children are all outside the current slice: `rolesByChild` has
  // no entry for its rows and `childBlocks` none for the role. The fold must
  // then report zero children rather than adopting whatever it finds.
  it('reports no children for a role that grants nothing on screen', () => {
    const soloRows = [{ id: 'BR1', displayName: 'Business Role 1' }, { id: 'G4', displayName: 'Group 4' }];
    const soloAnalysis = analyseRoleRows(soloRows, childrenByRole);
    const out = buildRoleLayout(soloRows, soloAnalysis, new Set(['BR1']));
    expect(out.visibleRows.map((r) => r.id)).toEqual(['BR1', 'G4']);
    expect(out.foldedChildRows.size).toBe(0);
  });
