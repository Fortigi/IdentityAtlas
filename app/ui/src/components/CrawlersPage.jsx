...
const CRAWLER_TYPES = [
  ...
  {
    id: 'midpoint',
    name: 'midPoint (Evolveum)',
    description: 'Sync orgs, roles, services, users, accounts, assignments, and certification reviews directly from the midPoint REST API',
    available: true,
  },
  {
    id: 'custom',
...
function MidpointWizard({ onComplete, onCancel, initialConfig, isEdit, authFetch }) {
  ...
  // ── Mapping tables (defaults reproduce the historical hardcoded behaviour) ──
  ...
  // ── Live discovery (archetypes / subtypes from the midPoint node) ──
  const [disco, setDisco] = useState(null);            // null = not fetched yet
  ...
  const fetchDiscovery = async () => {
    ...
    try {
      ...
      const r = await authFetch('/api/admin/midpoint/discover', {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Credential fields — blank secrets are omitted (kept as-is in edit mode)
