...
// Current auth state summary — enabled/disabled banner + tenant/client/roles grid.
function CurrentStateCard({ enabled, tenantId, clientId, requiredRoles, error, reload }) {
  return (
...
// How to change auth settings via the in-container CLI (Docker deployments only).
function ChangeSettingsSection({ tenantId, clientId }) {
...
// Entra ID app-registration walkthrough (Docker deployments only).
...
export default function AuthSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isAzure && <ChangeSettingsSection tenantId={tenantId} clientId={clientId} />}

      {!isAzure && <SetupWalkthrough />}
    </div>
  );
}
