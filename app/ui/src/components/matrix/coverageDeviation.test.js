...
describe('cellDeviation', () => {
  ...
  // `managedApMap.get(...)` returns undefined for a cell with no entry at all,
  // which is the overwhelmingly common case on a real grid — the caller's `|| []`
  // is what makes that read as "no role covers this" rather than as a gap on
  // every empty cell in the matrix.
  it('treats a missing coverage list the same as an empty one', () => {
    expect(deviate({ types: undefined, apIds: undefined, resourceKey: 'G1' })).toBe(NO_DEVIATION);
    expect(deviate({ types: types('Direct'), apIds: null, resourceKey: 'G1' })).toBe(NO_DEVIATION);
  });
