...
// Wraps the cell in a row with its own click handler, so a badge activation
// that failed to stop propagation would reach a real listener.
const renderBadgeCell = (props = {}) =>
  renderWithProviders(
...
describe('MatrixCell — Indirect badge', () => {
  it('is operable by Enter and Space, not just focusable', async () => {
    const onExplainInherited = vi.fn();
    renderBadgeCell({ membershipTypes: new Set(['Indirect']), onExplainInherited });
  ...
  it('leaves an Indirect badge inert when no explainer handler is supplied', () => {
    renderBadgeCell({ membershipTypes: new Set(['Indirect']) });
  ...
  it('leaves Direct and Eligible badges non-interactive even with a handler', () => {
    renderBadgeCell({ membershipTypes: new Set(['Direct', 'Eligible']), onExplainInherited: vi.fn() });
  ...
  it('renders an unknown membership type as a plain "?" swatch', () => {
    renderBadgeCell({ membershipTypes: new Set(['Bogus']), onExplainInherited: vi.fn() });
    expect(screen.getByText('?')).toBeInTheDocument();
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('renders an empty cell with no badges at all', () => {
    renderBadgeCell({ membershipTypes: new Set() });
