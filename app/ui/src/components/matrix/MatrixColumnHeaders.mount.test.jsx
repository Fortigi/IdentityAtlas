...
describe('MatrixColumnHeaders accounts row', () => {
  ...
  // Pinned to the rotated header, so every grouping level is exactly one row.
  ...
  it('adds the accounts row only while an identity is expanded', () => {
    const plain = renderHeaders([{ attribute: 'department' }], { headerMode: 'rotated' });
...
describe('MatrixColumnHeaders axis corner controls', () => {
  ...
  it('puts the column-axis corner on the first cross-table row, grown to fit it', () => {
    const { container } = renderHeaders(twoAttrs, { ...corners, headerMode: 'cross' });
    const rows = [...container.querySelectorAll('thead tr')];
    // Finance/Ops on the first level + Payroll/Logistics on the second → four value rows, then names.
    expect(rows).toHaveLength(5);
    expect(rows[0]).toHaveTextContent('fold columns');
    expect(rows.slice(1)).not.toContainEqual(expect.objectContaining({ textContent: expect.stringContaining('fold columns') }));
    expect(rows[0].querySelector('th').style.height).toBe(`${CORNER_ROW_H}px`);
    expect(rows[1].querySelector('th').style.height).toBe(`${VALUE_ROW_H}px`);
    // The taller first row is part of the sticky offset, so the names row still pins at top:0.
    expect(container.querySelector('thead').style.top).toBe(`-${3 * VALUE_ROW_H + CORNER_ROW_H}px`);
  });
