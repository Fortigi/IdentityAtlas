  ...
  const result = renderWithProviders(
    h('table', null, h('tbody', null,
      h(MatrixGroupRow, {
        ...
        // Empty by default: the deviation tests below assert on a subject that
        // does NOT hold what the role assigns, and every test that wants a
        // membership passes its own.
        memberships: new Map(),
        ...
        apGroupMap: new Map(),
        managedFilter: 'all',
        // These tests are about the business-role layer, which only a matrix
        // that opted into business-role rows has. The one test that asserts
        // what the DEFAULT matrix shows overrides this.
        showBusinessRoles: true,
...
describe('MatrixGroupRow — how a cell deviates from what its role assigns', () => {
  ...
  // The default matrix keeps the markers it has always had. "More than the role
  // assigns" belongs to the business-role layer and arrives with it, so a matrix
  // that did not ask for that layer is not quietly given a new red mark.
  describe('in a matrix without business-role rows', () => {
    const plain = { showBusinessRoles: false, foldableRoles: new Set() };

    it('still marks a membership the role assigns and the subject lacks', () => {
      renderRow(cellRow, {
        ...covered, ...plain,
        apGroupMap: new Map([['G1|br1', 'Member']]),
      });
    ...
    it('does not mark a standing membership on a just-in-time grant', () => {
      renderRow(cellRow, {
        ...covered, ...plain,
        apGroupMap: new Map([['G1|br1', 'Eligible Member']]),
        memberships: new Map([['G1|u1', new Set(['Direct'])]]),
