    ...
    <thead className="sticky z-30" style={{ top: `-${groupingOffset}px` }}>
      ...
      {attrRows.map((row, rowIdx) => (
        <tr key={row.attribute + rowIdx}>
          ...
          <th className="border-b border-r border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800" style={{ minWidth: '180px' }} />
      ...
      <tr>
        ...
        {/* Contexts is display-only — filtering by context lives in the matrix
            filter wizard's context control, so this header carries no sort. */}
        <th className="sticky top-0 z-20 border-b border-r border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 px-2 py-1 text-xs text-gray-600 dark:text-gray-400 font-medium text-left"
