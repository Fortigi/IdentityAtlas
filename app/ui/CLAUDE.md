...
- `components/reports/formRenderers.js` — `resolveFormRenderer(form)`, the report form → renderer map behind `ReportsPage.jsx`. Reports are self-describing (the API sends `form` + `columns`), so a new report needs **no UI change at all** and nothing here may key off a report *name* — a new presentation form is one entry in this map plus its renderer. See [`docs/architecture/reports.md`](../../docs/architecture/reports.md)
...
Why a map and not the old inline `page === 'x' ? <X/> : …` chain: **v8 does not line-instrument the arms of a JSX ternary**, so those lines carried no coverage record and every routing edit tripped the `Diff coverage: UI` gate on an un-coverable line ([#669](https://github.com/Fortigi/IdentityAtlas/issues/669)). Map entries + render-function statements are ordinary instrumented code the registry test covers.

**A page shell that is "mostly JSX" is still coverable — mount it.** It was previously assumed that any component built around one big JSX `return` was un-instrumentable, and that a `Diff coverage: UI` red on such a file was expected and unfixable. That is not what v8 does ([#725](https://github.com/Fortigi/IdentityAtlas/issues/725)).

v8 emits line records for the **statements** in a file — the top-level constants, the hooks, and the bodies of the callbacks embedded in the JSX (a `renderEntityCell`, an `onClick`). It emits none for the surrounding markup lines. Those markup lines are invisible to `diff_cover` in both directions: they never count as covered and never count as missing. The lines it *does* flag are precisely the instrumented ones — and those run as soon as something renders the component.

So a shell reading 0% means **nothing mounts it**, not that it cannot be measured. `DepartmentDetailPage.jsx` (97%) and `RotatedMatrixView.jsx` (93%) are the same shape and are covered, by `*.mount.test.jsx` files. Write one — see the harness under "UI mount tests" and the existing 35 examples. Beware the trap that hid this: a `*.test.js` that `readFileSync`s the component and asserts on the source *text* looks like a test, executes nothing, and leaves the file at 0.

**The single accepted exception is `App.jsx`**, which `diff_cover` excludes by name. It is the 679-line app shell — the detail-tab dispatch and matrix fallback that need surrounding app state deliberately stayed there when the routing switch moved to `pageRegistry.jsx`. Everything else gets a mount test.
