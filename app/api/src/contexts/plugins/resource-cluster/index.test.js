...
describe('resource-cluster plugin run()', () => {
  ...
  it('clusters resources that share a significant token, regardless of wrapping', async () => {
    const db = makeDb(resourcesByName(
      ...
      'AG_AzureTeam_Axiom_Readers',
      'SG_FINANCE_BookKeepers', // wrong cluster — shouldn't land in AXIOM
    ));
    const result = await plugin.run({ minMembers: 4 }, { db });
    const byExt = new Map(result.contexts.map(c => [c.externalId, c]));
    expect(byExt.has('token:axiom')).toBe(true);
    // FINANCE has only 1 resource — below minMembers of 4.
    expect(byExt.has('token:finance')).toBe(false);
    // AXIOM cluster attaches to the root, not as a top-level node.
    expect(byExt.get('token:axiom').parentExternalId).toBe('root');
    // All four AXIOM resources are members, Finance resource is not.
    const axiomMembers = result.members.filter(m => m.contextExternalId === 'token:axiom');
    expect(axiomMembers).toHaveLength(4);
  });
  ...
  it('applies additionalStopwords on top of defaults', async () => {
    ...
    // Without the extra stopword, "rol" (4 hits) and "axiom" (4 hits) both cluster.
    ...
    expect(baselineIds).toContain('token:axiom');

    // With rol in additionalStopwords, only axiom survives.
    const tuned = await plugin.run(
      { minMembers: 4, additionalStopwords: ['rol'] },
      { db },
    );
    const tunedIds = tuned.contexts.map(c => c.externalId).sort();
    expect(tunedIds).toEqual(['root', 'token:axiom']);
  });

  it('member rows reference the right contextExternalId', async () => {
    const db = makeDb(resourcesByName(
      'sg_axiom_admins', 'sg_axiom_readers', 'sg_axiom_writers', 'sg_axiom_users',
    ));
    const result = await plugin.run({ minMembers: 4 }, { db });
    // Every member row points at 'token:axiom'.
    for (const m of result.members) {
      expect(m.contextExternalId).toBe('token:axiom');
