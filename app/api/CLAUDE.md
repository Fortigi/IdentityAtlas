...
- `secrets/vault.js` — encrypted secret storage/retrieval. Every read / existence check / delete takes the row's **scope** (`getSecret(id, scope)`); a row in another scope is treated as absent. Crawler credentials go through `secrets/crawlerSecrets.js` (all four credential fields vaulted per config; which config's credentials a job receives comes only from `CrawlerJobs."configId"`, never from job JSON). Master-key rotation: `cli/rotate-master-key.js`
