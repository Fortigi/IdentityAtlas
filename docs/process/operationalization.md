...
**Sidekick baseline.** A pool sidekick needs more than the fresh-clone default (`git` + `docker`): also **`gh`, Node 22, `unzip`, `jq`, `build-essential`** (the agent uses `gh` for the PR and `npx vitest` to self-validate; `claude-code-action` needs `unzip` for its bundled Bun runtime). Enrolling a new member = install that baseline + register a `dor-build` self-hosted runner (systemd, auto-start) + a stable `skN` label. The runner holds no Traefik/authentik creds — an infra-managed central Traefik routes `N.build` → the sidekick's `:3001`.
