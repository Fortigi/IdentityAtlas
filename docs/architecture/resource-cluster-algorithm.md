...
`SG_APP_AXIOM_FINANCE_Admins_P` contributes to both a **AXIOM** cluster
...
- Your organisation uses consistent tokens (e.g. every AXIOM-related
  group has "AXIOM" somewhere in its name, even if the surrounding
...
`additionalStopwords=["rol","azure","sch","appx","svcadmin","helios","sandbox","gedelegeerdproductowner","commissie"]`):
...
With `minMembers=4`, only the **AXIOM** cluster survives. With
...
ORG                 227
IGA                 210
AXIOM               176   ← was 6 with the old stem-based algorithm
```

The same dataset with the previous stem-based algorithm produced 5
clusters (1 root + 4 stems), one of which was "app_axiom" at 6 members.
The token-based algorithm found 176 AXIOM-named resources — the ones
the stem stripper missed because they didn't match the fixed `(SG|DL|AG|…)_`
prefix whitelist.

## 6. Limitations

- **No fuzzy matching.** `AXIOM` and a typo'd `AXOIM` are different
