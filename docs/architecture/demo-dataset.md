...
| `DemoVolume.ps1` | **Opt-in** high-cardinality slice — see [The volume slice](#the-volume-slice-opt-in) |
...
Everything described on the rest of this page is the standard dataset: 39 resources, hand-reasoned, pinned to exact counts by `Verify-DemoDataset.ps1`. `DemoVolume.ps1` is the one part that is **not** generated by default. It runs only with `Generate-DemoDataset.ps1 -IncludeVolume`, and appends:
