...
- **On Azure** there is no VNet in this deployment shape, so the Container App has public ingress
  protected by a per-deployment API key (generated by the template, never entered by hand). It holds
...
- generates an API key per deployment and gives it to both sides;
