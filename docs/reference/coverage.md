...
|-------|------|--------|--------|------------|-----------|----------|---------------|
| [API (Node / Vitest — unit + contract)](../coverage/api/index.html) | 89.0% | 79.2% | 89.5% | 3.8 / 20 | 2.4 / 15 | 86.0% | 8,318 / 9,337 |
| [UI (React / Vitest)](../coverage/ui/index.html) | 82.5% | 72.6% | 71.3% | 2.8 / 28 | 1.1 / 15 | 69.5% | 6,531 / 7,913 |
| [PowerShell (Pester)](../coverage/powershell/index.html) | 91.0% | — | 96.7% | 3.8 / 15 | 4.0 / 15 | 100.0% | 5,955 / 6,540 |
...
- **Mutation is scoped.** Mutation testing covers 14 file(s) of 237 — 8% of the suite's coverable lines. It describes that subset — not the suite — and is not comparable with the suite-wide line figure on the same row. **The score itself is older than that scope:** it was measured over 6 file(s), before the current list was committed. Mutation runs are regenerated on their own schedule, so the percentage catches up on the next run.
- **method coverage (71.3%) sits below line coverage (82.5%)** — roughly a third of functions are never invoked, while the ones that are get exercised well. Typically components rendered but not interacted with: the untested part is event handlers, callbacks and conditional render paths.
...
- **Mutation is scoped.** Mutation testing covers 116 file(s) of 145 — 93% of the suite's coverable lines, using 4 mutation operators. It describes that subset — not the suite — and is not comparable with the suite-wide line figure on the same row. **The score itself is older than that scope:** it was measured over 90 file(s), before the current list was committed. Mutation runs are regenerated on their own schedule, so the percentage catches up on the next run.
...
_Generated 2026-09-12 14:56 UTC from commit `cd9159d4`._
