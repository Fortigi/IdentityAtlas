...
- Fixed: CRA fetch now queries per-identity using `Identity/Id eq {Id}` instead of a single paginated scan, matching the endpoint's natural page size of 1,000 and server-side indexing; result grows from 8,582 to 37,953 records covering all 332 identities completely
