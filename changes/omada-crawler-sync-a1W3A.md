...
- Fixed: OData pagination now stops on an empty page instead of a short page — the Builtin CalculatedAssignments endpoint returns variable-size pages even when more records remain, so the previous stop condition (`page.Count < pageSize`) cut the result off at ~98 records; changing to `page.Count == 0` correctly retrieves all 8,582+ active CRAs across all connected systems
