...
- Fixed: matrix was empty after loading demo data in portable mode — `REFRESH MATERIALIZED VIEW CONCURRENTLY` is not supported by PGlite's single-process WASM runtime; desktop mode now always uses plain `REFRESH`.
