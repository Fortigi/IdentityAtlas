...
.DESCRIPTION
    ...
    Update-CrawlerProgress) — see test/unit/EntraIDCrawlerPhases.Tests.ps1. This
    also pulls a large amount of cyclomatic complexity out of the entry point's
    top-level script body (the untestable I/O-on-load shell).

    This file holds the self-contained "leaf" phases that consume no state produced
    by an earlier phase and return nothing consumed by a later one: they take only
    -SystemId (+ the shared -Timings accumulator) and emit ingest calls.
#>
