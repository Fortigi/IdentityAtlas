...
function Get-SystemPrincipals {
    param([int]$SystemId)
    $resp = Get-Atlas "/users?systemId=$SystemId&limit=200"
    if ($resp.users) { return @($resp.users) }
...
$users = @(
    @{ id = $uAlice; userName = "alice.$runTag"; displayName = "Alice $runTag"; active = $true
       userType = 'employee'; department = 'Finance'; title = 'Analyst'
...
try {
    ...
    try {
        ...
        $hasSelected   = ($ext.department -eq 'Finance')
        $hasUnselected = ($null -ne $ext.title)          # 'title' was never selected
        Write-Result 'Scim/Data — only the opt-in attribute is stored' ($hasSelected -and -not $hasUnselected) `
            "(department: '$($ext.department)', title present: $hasUnselected — expected 'Finance' / False)"

        Write-Result 'Scim/Data — core mapping populated (display name, e-mail, name parts)' `
            ($alice.displayName -eq "Alice $runTag" -and $alice.email -eq "alice.$runTag@example.com" -and $alice.givenName -eq 'Alice') `
            "(display: '$($alice.displayName)', email: '$($alice.email)' — the primary e-mail, not the first)"
    ...
    try {
        ...
        Write-Result 'Scim/Scoping — second SCIM system ingested' (($otherJob.status -eq 'completed') -and ((Get-SystemPrincipals -SystemId $otherSystemId).Count -eq 1)) `
            "(system $otherSystemId)"
        ...
        Write-Result 'Scim/Idempotency — a repeated run updates rather than duplicates' `
            (($second.status -eq 'completed') -and ($afterSecond.Count -eq 3)) "($($afterSecond.Count) principal(s) after run 2; expected 3)"
        ...
        Write-Result 'Scim/Reconcile — a user removed at the source is deleted here' `
            ...
            "($($afterDelete.Count) principal(s), Bob gone: $bobGone — expected 2 / True)"
        ...
        Write-Result 'Scim/Reconcile — an emptied endpoint clears its rows and still succeeds' `
            ...
            "(status: $($fourth.status), $($afterEmpty.Count) principals, $($resAfterEmpty.Count) groups — all expected 0)"
    ...
    try {
        ...
        Write-Result 'Scim/Paging — every page is walked (25 users at count=10)' `
            (($pagingJob.status -eq 'completed') -and ($pagedPrincipals.Count -eq 25)) "($($pagedPrincipals.Count) principal(s); expected 25)"
        ...
        Write-Result 'Scim/Delta — a delta request completes as a full sync' `
            (($deltaJob.status -eq 'completed') -and ($afterDelta.Count -eq 25)) "(status: $($deltaJob.status), $($afterDelta.Count) principal(s))"
    } finally {
