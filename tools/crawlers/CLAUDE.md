...
- Keys starting with `_` are reserved for the dispatcher — don't use them for your own config. Injected today: `_syncMode` (`delta` / `full`), `_scheduledByConfigId` (the `CrawlerConfigs` row a scheduled run came from), `_scheduleIndex`, and `_configName` (the crawler's own `displayName`, absent when the crawler has no name or the job was queued with an inline config). A crawler that registers an Identity Atlas system names it after `_configName` rather than its own type literal, so two crawlers of one type don't produce two identically-named systems (#1207/#1240). Don't re-derive that rule — call `Get-CrawlerSystemName` from `tools/crawlers/shared/Get-CrawlerSystemName.ps1` (`-TypeDefault` is your type's label, `-ConfigName` is `_configName`, `-SystemName` is the wizard override if your type has one). It also treats a stored override that is an exact copy of your type default as unset, which is what lets a config saved before the rule existed follow the crawler name without being re-saved. **Only the system for your own endpoint/tenant follows the crawler name** — systems you discover *through* it (Omada's connected systems, midPoint's resources) keep the name their source gives them.
...
| `tools/crawlers/shared/Get-CrawlerSystemName.ps1` | `Get-CrawlerSystemName` — the one naming rule for the system a crawler registers for its own endpoint: explicit override ▸ crawler name (`_configName`) ▸ type default, with a stored override equal to the type default treated as unset (#1240) |
